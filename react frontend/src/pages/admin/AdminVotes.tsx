...
const AdminVotes: React.FC = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (openDropdown && !target.closest('.dropdown-container') && !target.closest('.dropdown-menu')) {
    ...
    const handleDropdownToggle = (voteId: string) => {
        if (openDropdown === voteId) {
            setOpenDropdown(null);
        } else {
            const button = dropdownButtonRefs.current[voteId];
            if (button) {
        ...
        <div className="p-6">
            ...
            <div className="mb-6">
                <nav className="text-sm text-gray-500 mb-2">
                    ...
                    <span className="text-gray-900 dark:text-white">Votings</span>
                </nav>
                <div className="flex items-center justify-between">
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Votings Management</h1>
                    <button
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {paymentGateways.map((gateway: any) => (
                            <div key={gateway.id} className="p-4 border border-gray-200 rounded-lg">
                                ...
                                <p className="text-sm text-gray-600 dark:text-gray-400">ID: {gateway.pg_id}</p>
                        ...
                        {paymentGateways.length === 0 && (
                            <div className="col-span-full text-center py-8">
                                ...
                                <p className="text-gray-600 dark:text-gray-400">No payment gateways available</p>
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        <div className="min-w-full">
                            <table className="w-full table-auto">
                                ...
                                <tbody className="bg-white dark:bg-secondary-900 divide-y divide-gray-200 dark:divide-secondary-700">
                                    {votes.map((vote: any) => (
                                        <tr key={vote.id} className="hover:bg-gray-50 dark:bg-secondary-800">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 relative">
                                                <div className="dropdown-container">
                                                    <button
                                                        ref={(el) => (dropdownButtonRefs.current[vote.vote_id] = el)}
                                                        onClick={() => handleDropdownToggle(vote.vote_id)}
                                                        className="p-2 text-gray-400 hover:bg-gray-100 dark:hover:bg-secondary-700 rounded-lg transition-colors duration-200"
                                                        title="More actions"
                                                    >
                                                        <MoreVertical className="w-4 h-4" />
                                                    </button>

                                                    {openDropdown === vote.vote_id && createPortal(
                                                        <div className="dropdown-menu fixed z-[9999] w-56 bg-white dark:bg-secondary-900 rounded-xl shadow-2xl border border-gray-200 dark:border-secondary-700 max-h-[80vh] overflow-y-auto"
                                                            style={{
                                                ...
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    {paymentGatewayData.pg_id === gateway.pg_id && (
                                                        <div className="w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center">
                                                            <div className="w-2 h-2 bg-white dark:bg-secondary-900 rounded-full"></div>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                    ...
                    <div className="card-glass max-w-2xl w-full animate-scale-in">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Vote ID</label>
                                    <p className="text-sm text-gray-900 dark:text-white">{selectedVote.vote_id}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-gray-900 dark:text-white">{selectedVote.payment_mode}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Price per Vote</label>
                                    <p className="text-sm text-gray-900 dark:text-white">₦{selectedVote.price_per_vote?.toLocaleString() || 0}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Nominees</label>
                                    <p className="text-sm text-gray-900 dark:text-white">{selectedVote.nominees_count || 0}</p>
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Votes</label>
                                    <p className="text-sm text-gray-900 dark:text-white">{selectedVote.total_votes || 0}</p>
                                </div>
                            </div>
                            ...
                            {selectedVote.description && (
                                <div className="border-t pt-4">
                                    ...
                                    <p className="text-sm text-gray-600 dark:text-gray-400">{selectedVote.description}</p>
                                </div>
                            )}
                            ...
                            <div className="border-t pt-4">
                                ...
                                <div className="grid grid-cols-2 gap-4 text-sm">
                                    {selectedVote.nomination_start && (
                                        <div>
                                            <label className="font-medium text-gray-500 dark:text-gray-400">Nomination Start</label>
                                            <p className="text-gray-900 dark:text-white">{new Date(selectedVote.nomination_start).toLocaleDateString('en-US', {
                                                weekday: 'long',
                                                year: 'numeric',
                                                month: 'long',
                                                day: 'numeric'
                                    ...
                                    {selectedVote.nomination_end_date && (
                                        <div>
                                            <label className="font-medium text-gray-500 dark:text-gray-400">Nomination End</label>
                                            <p className="text-gray-900 dark:text-white">{new Date(selectedVote.nomination_end_date).toLocaleDateString('en-US', {
                                                weekday: 'long',
                                                year: 'numeric',
                                                month: 'long',
                                                day: 'numeric'
                                    ...
                                    <div>
                                        <label className="font-medium text-gray-500 dark:text-gray-400">Start Date</label>
                                        <p className="text-gray-900 dark:text-white">{new Date(selectedVote.start_date).toLocaleDateString('en-US', {
                                            weekday: 'long',
                                            year: 'numeric',
                                            month: 'long',
                                            day: 'numeric'
                                    ...
                                    <div>
                                        <label className="font-medium text-gray-500 dark:text-gray-400">End Date</label>
                                        <p className="text-gray-900 dark:text-white">{new Date(selectedVote.end_date).toLocaleDateString('en-US', {
                                            weekday: 'long',
                                            year: 'numeric',
                                            month: 'long',
                                            day: 'numeric'
                                    ...
                                    {selectedVote.release_result_date && (
                                        <div>
                                            <label className="font-medium text-gray-500 dark:text-gray-400">Results Date</label>
                                            <p className="text-gray-900 dark:text-white">{new Date(selectedVote.release_result_date).toLocaleDateString('en-US', {
                                                weekday: 'long',
                                                year: 'numeric',
                                                month: 'long',
                                                day: 'numeric'
